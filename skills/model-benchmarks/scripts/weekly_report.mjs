...
L.push('|---|---|---|---|');
for (const m of tier1.slice(0, 20)) L.push(`| \`${m.id}\` | ${fmtPrice(m)} | ${(m.ctx / 1024) | 0}K | ${benchStr(m.id)} |`);
L.push('');
L.push('## Tier 2 — cheap fliers, no AA data (trial only, never pin blind)');
L.push('');
L.push('| Route | $/1M in/out | ctx |');
L.push('|---|---|---|');
for (const m of tier2.slice(0, 20)) L.push(`| \`${m.id}\` | ${fmtPrice(m)} | ${(m.ctx / 1024) | 0}K |`);
L.push('');
